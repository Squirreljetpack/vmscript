...
vmscript - An accessibility layer for vfio setups.
...
Options:
  ...
  --no-looking-glass          Sets VM_LOOKING_GLASS to false, causing 'run' to start the client. Defaults to 'true' when conditions are met.
...
Subcommands:
  prepare [VM_MACHINE]
    Prepares the VM's XML configuration from a base xml at $VM_XML_DIR/$VM_MACHINE/$VM_MACHINE.xml (see dump subcommand) as follows:
    ...
    - merges additional XML definitions found in $VM_XML_DIR/$VM_MACHINE/*.xml
      ...
      - The XML definitions can either be fully defined (starting from <domain>, see vmscript _mergexml --help for more details) or the kind of device fragments accepted by virsh attach-device
  ...
  hooks [VM_MACHINE]
    Presents a menu interface for installing from a collection of QEMU hooks to the VM.
```